...
<div class="container m-auto text-center pt-15 pb-5">
    <h1 class="text-6xl font-bold">Edit Your Course</h1>
</div>
...
<div class="container mx-auto px-4 pt-12 pb-6">
    <form action="/video/{{ $video->slug }}" method="post" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <h1 class="text-4xl font-bold mb-6">Edit The Video From <span class="text-gray-700">{{ $video->course->title }}</span> Course</h1>
      <div class="mb-8">
        <label for="title" class="block text-lg font-medium text-gray-700 mb-2">Title</label>
        <input type="text" name="title" id="title" value="{{ $video->title }}" placeholder="Enter course title" class="w-full px-4 py-3 rounded-lg border-gray-300 focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50 transition duration-300">
      </div>
      <div class="mb-8">
        <label for="description" class="block text-lg font-medium text-gray-700 mb-2">Description</label>
        <textarea name="description" id="description" rows="6" placeholder="Enter course description" class="w-full px-4 py-3 rounded-lg border-gray-300 focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50 transition duration-300">{{ $video->description }}</textarea>
      </div>
      <div class="mb-8">
        ...
        <div class="flex flex-col items-center justify-center border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition duration-300">
          ...
          </div><input type="file" value="{{ $video->video_path }}" name="image" id="image" class="hidden">
      ...
      <div class="mb-8">
        ...
        <div class="flex flex-col items-center justify-center border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition duration-300">
          ...
          <input type="file" name="poster" value="{{ $video->image_path }}" id="poster" class="hidden">
      ...
      <input type="text" name="course_id" value="{{ $video->course->id }}" placeholder="course_id" class="w-full h-20 text-6xl rounded-lg shadow-lg border-b p-15 mb-5 hidden">
...
<form action="/video/{{ $video->slug }}" method="post" enctype="multipart/form-data">
    @csrf
    @method('PUT')
<input type="text" name="title" value="{{ $video->title }}" class="w-full h-20 text-6xl rounded-lg shadow-lg border-b p-15 mb-5">
<textarea  id="" name="description" placeholder="Description" class="w-full h-60 text-6xl rounded-lg shadow-lg border-b p-15 mb-5">
    {{ $video->description }}
</textarea>
<div class="py-15">
...
<input type="text" name="course_id" value="{{ $video->course->id }}" placeholder="course_id" class="w-full h-20 text-6xl rounded-lg shadow-lg border-b p-15 mb-5 hidden"> --}}
    {{-- <label for="course_id" class="block font-medium text-gray-700">Choose a Course:</label>

    <select id="course_id" name="course_id" class="block w-full max-w-xs mt-1 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300">
