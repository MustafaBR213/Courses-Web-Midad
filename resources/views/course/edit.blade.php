...
<div class="container m-auto text-center pt-15 pb-5 ">
    <h1 class="text-6xl font-bold mt-10">Edit Your Course</h1>
</div>


<div class="container mx-auto px-4 pt-12 pb-6">
    <form action="/courses/{{ $courses->slug }}" method="post" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <h1 class="text-4xl font-bold mb-6">Edit Your Course Information</h1>
      <div class="mb-8">
        <label for="title" class="block text-lg font-medium text-gray-700 mb-2">Title</label>
        <input type="text" name="title" id="title" placeholder="Enter course title" value="{{ $courses->title }}" class="w-full px-4 py-3 rounded-lg border-gray-300 focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50 transition duration-300">
      </div>
      <div class="mb-8">
        <label for="description" class="block text-lg font-medium text-gray-700 mb-2">Description</label>
        <textarea name="description" id="description" rows="6" placeholder="Enter course description" class="w-full px-4 py-3 rounded-lg border-gray-300 focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50 transition duration-300">{{ $courses->description }}</textarea>
      </div>
      <div class="mb-8">
        ...
        <div class="flex flex-col items-center justify-center border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition duration-300">
          ...
          <input type="file" name="image" value="{{ $courses->image_path }}" id="image" class="hidden">
        </div>
    </label>
      </div>
